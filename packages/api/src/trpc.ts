...
const isAuthed = t.middleware(async ({ ctx, next }) => {
  ...
  if (process.env.NODE_ENV === "development" && process.env.SKIP_AUTH === "true") {
    try {
      ...
      console.warn("Failed to create test user/org in database, using mock data:", error);
      
      // 데이터베이스 접근 실패 시 메모리 기반 목 데이터 사용
      const mockOrg = {
        id: "org_test_001",
        clerkOrgId: "test_clerk_org_id",
        name: "테스트 조직",
      ...
      return next({
        ctx: {
          ...ctx,
          auth: { userId: mockUser.clerkId, orgId: mockOrg.clerkOrgId },
          user: mockUser,
          organizationId: mockOrg.id,
        },
      });
